/*  we have to create a sevice.
    Using this service we will run a background job that will check the 
    scheduled tasks every second.

    And when it is scheduled for current time it will publish them using the zernio

    In this file we have to create the cron job. for that we had to add a package "node-cron"
*/
...
export const initScheduler = () =>{
        ...
        try {
            ...
            for (const post of postsToPublish){
                try {
                    ...
                    if(!publishedPost){
                        throw new Error("Failed to ge the post object from Zernio response");
                    }
    ...
    console.log("Scheduler service intialized.");
}
